...
6. ⚠ **PASS 6 RAN AND RETURNED `blocker` (reviewer Y, opus): 3 blockers, 3 majors, 2 minors.** It broke
   the closed-set apparatus a **THIRD** time — reviewer X's own exploit, **moved one paragraph** — and
   passed a **fabricated owner ruling** through all five oracles. It also found unsourced owner authority
   written into `~/Documents/Architect.md` itself, the one file no oracle reads.
7. **The `Severity` spec change is REVERTED**; the owner's original signature stands, and the node logs the
   merged issue set instead. See **ADJ-19**.
8. ⚠ **F-1 AND F-2 ARE OPEN AND DELIBERATELY UNPATCHED — see ADJ-20.** A third break in three passes is
   treated as evidence the apparatus needs a **different design**, not a fourth patch. The proposal — that
   the closed-set section be **generated by the assembly step from the signature**, not authored — spans
   elements 1 and 4 and is with the owner. **`records/harness-run-2026-07-30.txt` records both forgeries as
   LIVE**, built from the current artifact, every oracle clean. **A green harness does not mean the
   apparatus works.**
9. **Reviewer Z has not been dispatched.** Everything in ADJ-17…ADJ-21 is **unreviewed**.
