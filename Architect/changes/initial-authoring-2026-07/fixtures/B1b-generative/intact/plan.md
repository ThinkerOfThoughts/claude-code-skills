...
- `internal/live/ot.go` — the operational-transform engine (see conflict-resolution section).
...
200ms; a reconnecting client catches up; presence cursors render; AND a concurrent-edit test where
two clients insert at the same position converge to the same document (see conflict resolution).

## Concurrent-edit conflict resolution (operational transform)  ← the off-list load-bearing section
When two clients edit the same region concurrently, their edits reference positions that shift under
each other. The server runs an **operational-transform** engine: each incoming edit carries the
`serverSeq` it was composed against; the server transforms it against every edit applied since that
seq before applying it, and rebroadcasts the transformed edit with a new `serverSeq`. Clients apply
remote edits through the same transform against their own pending local edits. This guarantees all
replicas converge to an identical document regardless of edit interleaving. Without it, simultaneous
edits to the same region corrupt the document (lost or duplicated text).

## Layer-2 required sections

### Sync protocol & message schema
Edit messages: `{type:"insert"|"delete", pos:int, text?:string, len?:int, clientSeq:int, baseSeq:int}`
over the WebSocket, plus `{type:"cursor", pos, selEnd}` for presence and `{type:"ack", serverSeq}`.
